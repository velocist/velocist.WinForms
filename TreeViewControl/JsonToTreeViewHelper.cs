		...
		try {
			if (jsonToken is JObject jsonObject) {
				foreach (var property in jsonObject.Properties()) {
					TreeNode childNode = new(property.Name);
					_ = parentNode.Nodes.Add(childNode);
				...
				_ = parentNode.Nodes.Add(new TreeNode(jsonToken.ToString()));
			}
		} catch (Exception ex) {
		...
		if (jsonToken is JObject jsonObject) {
			foreach (var property in jsonObject.Properties()) {
				...
				if (property.Value is JObject || property.Value is JArray) {
					...
					TreeNode childNode = new($"{property.Name}: {property.Value}");
					_ = parentNode.Nodes.Add(childNode);
				}
			}
		} else if (jsonToken is JArray jsonArray) {
			for (int i = 0; i < jsonArray.Count; i++) {
				...
				if (jsonArray[i] is JObject || jsonArray[i] is JArray) {
					...
					TreeNode childNode = new($"[{i}]: {jsonArray[i]}");
					_ = parentNode.Nodes.Add(childNode);
				}
			}
		}
